...
export default function Feedback() {
  const message : React.RefObject<HTMLInputElement|null> = React.useRef(null)
  const messageHandler = (e:HTMLFormElement)=>{
    const text:string = message.current?.value
    if(text != ''){
  ...
  return (
    <div className='w-3/4 h-16 p-2 mt-6 rounded-xl bg-violet-800 '>
            <form className='flex' onSubmit={(e:any)=>messageHandler(e)}>
                <input type='text' className='font-mono font-normal w-2/3 p-2 m-1' placeholder='Enter your message:' ref={message} onChange={(e:any)=>changeHandler(e)}></input>
