...
export default function ProjectForm({setShow}:any) {
  ...
  const onSubmit = async (data:TPortfolioSchema)=>{
    // post action
                ...
                <div className="scroll-layout w-full h-3/4 overflow-y-scroll flex justify-between">
                  <div className="w-1/2 mx-8 pr-24 ">
                    <h2 className="text-xl font-normal text-zinc-600 mt-6 mx-2 flex items-center">id:
                      {/* project id */} 
                      <input value={uuid} {...register('id',{required:"ID has not Set!!"})} className="text-zinc-400 px-2 text-xl" ></input>
                    </h2>
                    {errors.id && (<p className="text-sm text-red-600 font-semibold">{`${errors.id.message}`}</p>)}
                    <h2 className="text-xl font-medium text-zinc-600 mt-6 mx-2">Name Project:</h2>
                    {/* name ref */}
                      <input {...register('name',{required:"Please choose specific name for Project."
                              ,minLength:{value:4,message:'your project name must be have 4 character'}})} type="text" placeholder="name" 
                        className="w-full h-10 ml-6 my-4 px-2 text-lg font-light ring-1 ring-zinc-400 rounded-md"/>
                        {errors.name &&  (<p className="text-sm text-red-600 font-semibold">{`${errors.name.message}`}</p>)}
                    <h2 className="text-xl font-medium text-zinc-600 mt-4 mx-2">Description Project:</h2>
                    {/* description ref */} 
                      <input  {...register('description',{required:"Please write description for your Project!",
                                minLength:{value:12,message: 'Please write more...'}})} placeholder="information about project" 
                                className="w-full overflow-y-scroll min-h-44 max-h-80 
                  ...
                  <div className="w-full h-14 flex justify-end items-center mx-2 px-6">
                    <button className="w-24 h-10 p-1 text-xl rounded-lg mx-4 text-center
                        text-zinc-400 ring-1 ring-zinc-400 font-medium " 
                        onClick={clickHandler}>Cancel</button>
                    <button type="submit" className="w-20 h-10 p-1 text-xl rounded-lg 
                        mx-4 text-center text-white font-medium 
                        bg-dashboard-orange disabled:bg-gray-400 disabled:border-gray-400" disabled={isSubmitting}>Save</button>
                  </div>
              </form>
            </div>
    </main>
